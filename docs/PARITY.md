...
163 rows. Each entry is the capability's `inventory_row_id` and its recorded reason, verbatim from the matrix (`final_method_note`, falling back to `tolerance_justification`, falling back to "no note recorded" if neither is recorded).
